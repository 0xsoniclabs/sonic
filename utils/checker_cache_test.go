...
func TestCachedChecker_WrapCheck_UsesProvidedCache(t *testing.T) {
	cache := NewCheckerCache[bool](10)

	res := WrapCheck(cache, func(tx *types.Transaction) bool {
		return true
	})
...
func TestCachedChecker_ReturnsCachedValueWithoutCallingCheckerFunction(t *testing.T) {
	...
	expectNeverCalled := func(int hasheableInteger) bool {
		t.Fatal("unexpected call to the underlying checker")
		return false
	}
	check := WrapCheck(cache, expectNeverCalled)

	require.True(t, check.Check(hasheableInteger(1)))
	require.False(t, check.Check(hasheableInteger(2)))
}
...
func TestCachedChecker_CheckerIsGeneric(t *testing.T) {
	...
	require.True(t,
		WrapCheck(cache, func(*types.Transaction) bool {
			callCount++
			return true
		}).Check(types.NewTx(&types.LegacyTx{})))
	require.Equal(t, 3, callCount)
}

func TestCachedChecker_isSponsored_NonCachedValue_FetchesNewValue(t *testing.T) {
	cache := NewCheckerCache[bool](1024)
	...
	require.True(t, cachedChecker.check(i, timePoint))
	require.Equal(t, 1, callCount)
	...
	require.True(t, cachedChecker.check(i, timePoint.Add(time.Millisecond)))
	require.Equal(t, 1, callCount)
}

func TestCachedChecker_isSponsored_OutdatedEntry_FetchesNewValue(t *testing.T) {
	...
	cache.put(i.Hash(), checkerEntry[bool]{
		validUntil:       timePoint.Add(validInterval),
		validityDuration: validInterval,
		value:            true,
	})

	timePoint = timePoint.Add(validInterval + time.Millisecond)
	callCount := 0
	checker := func(tx hasheableInteger) bool {
		callCount++
		return false
	...
	cachedChecker := WrapCheck(cache, checker)
	require.False(t, cachedChecker.check(i, timePoint))
	require.Equal(t, 1, callCount)
	...
	require.Equal(t, entry.validUntil, timePoint.Add(entry.validityDuration))
	require.Equal(t, entry.validityDuration, 400*time.Millisecond) // 200ms * 2
}

func TestCachedChecker_isSponsored_ValidityDurationIsCapped(t *testing.T) {
	...
	// The entry is now outdated, so the underlying checker should be called.
	cachedChecker := WrapCheck(cache, checker)
	require.True(t, cachedChecker.check(i, now))
	require.Equal(t, 1, callCount)
