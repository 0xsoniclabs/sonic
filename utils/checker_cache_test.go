...
func TestCachedSubsidiesChecker_isSponsored_UsesValueFromCache(t *testing.T) {
	...
	now := time.Now()
	cache.put(tx1.Hash(), checkerEntry{
		validUntil: now.Add(time.Minute),
		value:      true,
	})
	cache.put(tx2.Hash(), checkerEntry{
		validUntil: now.Add(time.Minute),
		value:      false,
	})
	...
	require.True(t, cachedChecker.Check(tx1))
	require.False(t, cachedChecker.Check(tx2))
}
...
func TestCachedSubsidiesChecker_isSponsored_NonCachedValue_FetchesNewValue(t *testing.T) {
	...
	now := time.Now()
	checker.EXPECT().Check(tx).Return(true).Times(1)
	require.True(t, cachedChecker.check(tx, now))
	...
	require.True(t, cachedChecker.check(tx, now.Add(time.Millisecond)))
}
...
func TestCachedSubsidiesChecker_isSponsored_OutdatedEntry_FetchesNewValue(t *testing.T) {
	...
	cachedChecker := cache.Wrap(checker)
	checker.EXPECT().Check(tx).Return(false)
	require.False(t, cachedChecker.check(tx, now))
...
func TestCacheSubsidiesChecker_isSponsored_ValidityDurationIsCapped(t *testing.T) {
	...
	// The entry is now outdated, so the underlying checker should be called.
	cachedChecker := cache.Wrap(checker)
	checker.EXPECT().Check(tx).Return(true)
	require.True(t, cachedChecker.check(tx, now))
...
func TestUncachedChecker_DoesNotCacheChecker(t *testing.T) {
	ctrl := gomock.NewController(t)
	checker := NewMockChecker(ctrl)

	uncachedChecker := NewUnchachedChecker(checker.Check)

	tx := types.NewTx(&types.LegacyTx{Nonce: 1})

	now := time.Time{}

	// Two queries result of two calls being fowraded to the underlying checker
	checker.EXPECT().Check(tx).Return(true).Times(2)
	require.True(t, uncachedChecker.check(tx, now))
	require.True(t, uncachedChecker.check(tx, now))
}
