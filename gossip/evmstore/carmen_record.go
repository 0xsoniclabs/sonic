...
func (c *RecordCarmenStateDB) GetStateAndCommittedState(addr common.Address, hash common.Hash) (common.Hash, common.Hash) {
	value, commitedValue := c.CarmenStateDB.GetStateAndCommittedState(addr, hash)
	c.substateRecordAccess(addr)
	c.substateStorageAccess(addr, hash, value)
	return value, commitedValue
}
...
func (c *RecordCarmenStateDB) SetCode(addr common.Address, code []byte, reason tracing.CodeChangeReason) []byte {
	c.substateRecordAccess(addr)
	return c.CarmenStateDB.SetCode(addr, code, reason)
}
	...
	for addr, sa := range c.SubstatePreAlloc {
		...
		if found {
			for key := range ac {
				...
				_, value := c.GetStateAndCommittedState(comAddr, key)
