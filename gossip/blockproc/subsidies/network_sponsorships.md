...
The registry exposes a `getGasConfig()` function (selector `0x4b5c54c0`) that currently
returns three `uint64` values ABI-encoded as three 32-byte words (96 bytes total):

```
chooseFundGasLimit | deductFeesGasLimit | overheadToCharge
```

To support the tracking mode, one additional field is needed:

```
chooseFundGasLimit | deductFeesGasLimit | overheadToCharge | trackGasCost
```

The existing `getGasConfig()` function in the upgraded registry contract is extended to
return four 32-byte words (128 bytes). The node detects the registry version by the
length of the returned data:

| Response length | Interpretation |
|---|---|
| 96 bytes | Legacy registry — `trackGasCost` defaults to zero |
| 128 bytes | Extended registry — all four fields present |
| any other length | Error |

A zero `trackGasCost` on a legacy registry means the `track` transaction costs no gas,
which is the safe conservative behaviour (the transaction is still inserted but uses no
block gas).
...
`chooseFund` (32 vs 64 bytes) and `getGasConfig` (96 vs 128 bytes).
