...
//go:generate mockgen -source=bundle_precheck.go -destination=bundle_precheck_mock.go -package=bundle
...
func GetBundleState(
	...
	currentBlock := chainState.GetCurrentBlockHeight()
	...
	signer := types.LatestSignerForChainID(chainState.GetChainID())
	state := checkForNonceConflicts(bundle, signer, chainState.GetStateDB())
...
func checkForNonceConflicts(
	...
	if success := RunBundle(bundle, runner); !success {
		return BundleStatePermanentlyBlocked
	}
...
func getLowestReferencedNonces(
	...
	for _, tx := range bundle.Bundle {
		if IsTransactionBundle(tx) {
			bundle, _, err := ValidateTransactionBundle(tx, signer)
			if err != nil {
...
// _runner is an implementation of the TransactionRunner interface enabling the
// RunBundle function to be used for checking nonce conflicts without having
...
// the proper lifecycle management of the _runner.
type _runner struct {
	signer         types.Signer
...
func (r *_runner) Run(tx *types.Transaction) TransactionResult {

	// if the transaction is a nested bundle, process it as such
	if IsTransactionBundle(tx) {
		bundle, _, err := ValidateTransactionBundle(tx, r.signer)
		if err != nil {
			return TransactionResultInvalid
		}
		acceptedBackup := maps.Clone(r.acceptedSender)
		backup := r.nonceTracker.backup()
		if RunBundle(bundle, r) {
