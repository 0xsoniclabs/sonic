...
func TestMakeMaxRangeEndingAt_CreatesMaxRangeEndingAtGivenBlock(t *testing.T) {
	cases := map[string]struct {
		end              uint64
		...
		"end at MaxBlockRange minus 2 — still too close to zero": {
			end:              MaxBlockRange - 2,
			expectedEarliest: 0,
			expectedSize:     MaxBlockRange - 1,
		},
		"end at MaxBlockRange minus 1 — exact full range from zero": {
			end:              MaxBlockRange - 1,
			expectedEarliest: 0,
			expectedSize:     MaxBlockRange,
		},
		"end at MaxBlockRange — first block that shifts earliest past zero": {
			end:              MaxBlockRange,
			expectedEarliest: 1,
			expectedSize:     MaxBlockRange,
		},
		"end at MaxBlockRange plus 1": {
			end:              MaxBlockRange + 1,
			expectedEarliest: 2,
			expectedSize:     MaxBlockRange,
		},
		"end at 2 * MaxBlockRange": {
			end:              2 * MaxBlockRange,
			expectedEarliest: MaxBlockRange + 1,
			expectedSize:     MaxBlockRange,
		},
		"end near max uint64": {
			end:              math.MaxUint64 - 10,
			expectedEarliest: math.MaxUint64 - 10 - MaxBlockRange + 1,
			expectedSize:     MaxBlockRange,
		},
		"end at max uint64": {
			end:              math.MaxUint64,
			expectedEarliest: math.MaxUint64 - MaxBlockRange + 1,
			expectedSize:     MaxBlockRange,
		},
	}
	for name, c := range cases {
		t.Run(name, func(t *testing.T) {
			r := MakeMaxRangeEndingAt(c.end)
			require.Equal(t, c.expectedEarliest, r.Earliest)
			require.Equal(t, c.end, r.Latest)
			require.Equal(t, c.expectedSize, r.Size())
		})
	}
}

func TestMakeMaxRangeEndingAt_AlwaysIncludesEndBlock(t *testing.T) {
	// For every ending block the resulting range must contain it.
	for _, end := range []uint64{0, 1, MaxBlockRange - 2, MaxBlockRange - 1, MaxBlockRange, MaxBlockRange + 1, math.MaxUint64} {
		r := MakeMaxRangeEndingAt(end)
		require.True(t, r.IsInRange(end),
			"end=%d: range [%d,%d] should contain %d", end, r.Earliest, r.Latest, end)
	}
}

func TestMakeMaxRangeEndingAt_NeverExceedsMaxBlockRange(t *testing.T) {
	for _, end := range []uint64{0, 1, MaxBlockRange - 2, MaxBlockRange - 1, MaxBlockRange, MaxBlockRange + 1, 2 * MaxBlockRange, math.MaxUint64} {
		r := MakeMaxRangeEndingAt(end)
		require.LessOrEqual(t, r.Size(), uint64(MaxBlockRange),
			"end=%d: size %d exceeds MaxBlockRange %d", end, r.Size(), MaxBlockRange)
	}
}

func TestMakeMaxRangeEndingAt_IsMirrorOfMakeMaxRangeStartingAt(t *testing.T) {
	// When ending at block B, the earliest block produced by MakeMaxRangeEndingAt
	// should equal the start of MakeMaxRangeStartingAt(earliest).
	for _, end := range []uint64{MaxBlockRange, MaxBlockRange + 1, 2 * MaxBlockRange, math.MaxUint64 / 2, math.MaxUint64} {
		r := MakeMaxRangeEndingAt(end)
		r2 := MakeMaxRangeStartingAt(r.Earliest)
		require.Equal(t, r.Earliest, r2.Earliest,
			"end=%d: earliest mismatch", end)
		// Both ranges should have MaxBlockRange size when not near boundaries.
		require.Equal(t, r.Size(), r2.Size(),
			"end=%d: size mismatch", end)
	}
}
