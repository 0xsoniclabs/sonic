...
func (p *OperaEVMProcessor) Execute(txs types.Transactions, gasLimit uint64) []evmcore.ProcessedTransaction {
	...
	if txsOffset > 0 {
		for i := range processed {
			if processed[i].Receipt != nil {
				processed[i].Receipt.TransactionIndex += txsOffset
			}
		}
	}
...
func (p *OperaEVMProcessor) Finalize() (evmBlock *evmcore.EvmBlock, numSkipped int, receipts types.Receipts) {
	...
	for _, pt := range p.processedTxs {
		if pt.Receipt != nil {
			transactions = append(transactions, pt.Transaction)
			receipts = append(receipts, pt.Receipt)
		} else {
			numSkipped++
		}
	}
