...
type benchEnv struct {
	...
	prioritized    []common.Address // first benchPrioritizedSenders accounts
...
func BenchmarkPrioritize(b *testing.B) {
	...
	for _, n := range []int{10, 100, 1000, benchMaxBlockTxs} {
		...
		b.Run(fmt.Sprintf("EvmClassifier/n=%d", n), func(b *testing.B) {
			runPrioritize(b, txs, evmClassifier, env.cfg)
		})
		b.Run(fmt.Sprintf("Native/n=%d", n), func(b *testing.B) {
			runPrioritize(b, txs, nativeClassifier, env.cfg)
		})
	}
...
func runPrioritize(b *testing.B, txs types.Transactions, classifier Classifier, cfg Config) {
	b.ReportAllocs()
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		if got := Prioritize(txs, classifier, cfg); len(got) != len(txs) {
...
func setupBenchEnv(b *testing.B, numAccounts, numPrioritized int) *benchEnv {
	...
	prioritized := make([]common.Address, numPrioritized)
