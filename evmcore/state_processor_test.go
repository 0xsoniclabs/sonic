...
func TestRunTransaction_GasSubsidiesEnabled_RunsRegularTransactionWithoutSponsorship(t *testing.T) {
	...
	got := runTransaction(context, tx, 0)
...
func TestRunTransaction_GasSubsidiesEnabled_RunsSponsorshipRequestWithSponsorship(t *testing.T) {
	...
	processed := runTransaction(context, tx, 0)
	...
	for name, test := range tests {
		t.Run(name, func(t *testing.T) {
			...
			if !test.shouldSkip {
				...
				any := gomock.Any()
				runner.EXPECT().Call(any, any, any, any, any).
					Return([]byte{31: 1}, uint64(0), nil) // indicates "covered"

				runner.EXPECT().runWithoutBaseFeeCheck(any, tx, any).Return(ProcessedTransaction{
					Transaction: tx,
				...
				runner.EXPECT().runWithoutBaseFeeCheck(any, any, any).Return(ProcessedTransaction{
			...
			got := runSponsoredTransaction(context, tx, 0)
...
func TestRunSponsoredTransaction_SponsorshipCoverageCheckFails_ReturnsASkippedTransaction(t *testing.T) {
	...
	runner.EXPECT().Call(any, any, any, any, any).Return(nil, uint64(0), issue)
...
func TestRunSponsoredTransaction_FailingCreationOfFeeDeduction_TransactionIsAcceptedWithoutFeeDeduction(t *testing.T) {
	...
	runner.EXPECT().runWithoutBaseFeeCheck(any, tx, any).Return(processed)
...
func TestRunSponsoredTransaction_TxIndexIsIncrementedForFeeDeductionTx(t *testing.T) {
	...
	txIndex := 7
	runner.EXPECT().runWithoutBaseFeeCheck(any, tx, txIndex).
		Return(ProcessedTransaction{
			Transaction: tx,
			Receipt:     &types.Receipt{},
		})

	runner.EXPECT().runWithoutBaseFeeCheck(any, gomock.Not(tx), txIndex+1).
		Return(ProcessedTransaction{
			Transaction: tx,
			Receipt:     &types.Receipt{},
		})
	...
	got := runSponsoredTransaction(context, tx, txIndex)
	require.Len(t, got, 2)
	require.Equal(t, tx, got[0].Transaction)
	require.NotNil(t, got[0].Receipt)
	require.NotNil(t, got[1].Transaction)
