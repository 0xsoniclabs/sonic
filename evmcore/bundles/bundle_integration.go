...
//go:generate mockgen -source=bundle_integration.go -destination=bundle_integration_mock.go -package=corebundles
...
func NewPreCheckChainAdapter(chainState coretypes.StateReader, stateDB state.StateDB) *preCheckChainAdapter {
	return &preCheckChainAdapter{
...
func (a *preCheckChainAdapter) GetLatestHeader() *coretypes.EvmHeader {
	return &a.chainState.CurrentBlock().EvmHeader
}

func (a *preCheckChainAdapter) Header(hash common.Hash, number uint64) *coretypes.EvmHeader {
