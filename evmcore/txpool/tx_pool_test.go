...
func (bc *testBlockChain) CurrentBlock() *coretypes.EvmBlock {
	bc.mu.Lock()
	defer bc.mu.Unlock()
...
func (bc *testBlockChain) Block(hash common.Hash, number uint64) *coretypes.EvmBlock {
	return bc.CurrentBlock()
}

func (bc *testBlockChain) Header(common.Hash, uint64) *coretypes.EvmHeader {
	return bc.CurrentBlock().Header()
}

func (bc *testBlockChain) HasBundleBeenProcessed(execPlanHash common.Hash) bool {
	return false
}
...
func (bc *testBlockChain) SubscribeNewBlock(ch chan<- coretypes.ChainHeadNotify) event.Subscription {
	bc.mu.Lock()
	defer bc.mu.Unlock()
...
func setupTxPoolWithConfig(config *params.ChainConfig) (*TxPool, *ecdsa.PrivateKey) {
	...
	pool := newTxPool(testTxPoolConfig, config, blockchain, testSubsidiesCheckerFactory, testBundleCheckerFactory)
...
func validateEvents(events chan coretypes.NewTxsNotify, count int) error {
...
func TestTransactionGapFilling(t *testing.T) {
	...
	events := make(chan coretypes.NewTxsNotify, testTxPoolConfig.AccountQueue+5)
...
func testTransactionQueueGlobalLimiting(t *testing.T, nolocals bool) {
	...
	pool := newTxPool(config, params.TestChainConfig, blockchain, testSubsidiesCheckerFactory, testBundleCheckerFactory)
	defer pool.Stop()
...
func TestTransactionPool_RejectsUnderTippedTransactions(t *testing.T) {
	...
	// Keep track of transaction events to ensure all executables get announced
	events := make(chan coretypes.NewTxsNotify, 32)
	sub := pool.txFeed.Subscribe(events)
	defer sub.Unsubscribe()
...
func TestTransactionPool_FeedAnnouncesChanges(t *testing.T) {
	...
	feed := make(chan coretypes.NewTxsNotify, 32)
...
func TestTxPool_ActivatingOsakaDropsTransactionsWithHighGas(t *testing.T) {
	...
	oldHeader := &coretypes.EvmHeader{Number: big.NewInt(4), Time: 4}
	newHeader := &coretypes.EvmHeader{Number: big.NewInt(5), Time: 5, BaseFee: big.NewInt(100)}
