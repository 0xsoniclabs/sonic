...
//go:generate mockgen -source=tx_pool.go -destination=tx_pool_mock.go -package=txpool
...
type subsidiesCheckerFactory func(
	rules opera.Rules,
	chain coretypes.StateReader,
	state state.StateDB,
	signer types.Signer,
) coresubsidies.SubsidiesChecker

// bundleCheckerFactory is a factory method to create a bundle checker instance.
// This facilitates testing of the TxPool by using injected mock implementations.
type bundleCheckerFactory func(
	rules opera.Rules,
	chain coretypes.StateReader,
	state state.StateDB,
	signer types.Signer,
) corebundles.BundleChecker
...
type TxPool struct {
	...
	chainHeadCh     chan coretypes.ChainHeadNotify
	...
	subsidiesCheckerCache   *coresubsidies.SubsidiesCheckerCache // Cache for subsidies check results

	bundleCheckerFactory bundleCheckerFactory // Factory to create a bundle checker instance
}

type txpoolResetRequest struct {
	oldHead, newHead *coretypes.EvmHeader
}
...
func NewTxPool(
	config TxPoolConfig,
	chainconfig *params.ChainConfig,
	chain coretypes.StateReader) *TxPool {
	return newTxPool(
		config, chainconfig,
		chain,
		coresubsidies.NewSubsidiesChecker, corebundles.NewBundleChecker)
}
...
func newTxPool(
	...
	pool := &TxPool{
		...
		all:             NewTxLookup(),
		chainHeadCh:     make(chan coretypes.ChainHeadNotify, chainHeadChanSize),
		...
		subsidiesCheckerFactory: subsidiesCheckerFactory,
		subsidiesCheckerCache:   coresubsidies.NewSubsidiesCheckerCache(-1), // use default size
...
func (pool *TxPool) SubscribeNewTxsNotify(ch chan<- coretypes.NewTxsNotify) event.Subscription {
...
func (pool *TxPool) HasBundle(execPlanHash common.Hash) bool {
	...
	for _, tx := range pool.all.txs() {
		if bundle.IsTransactionBundle(tx) {
			_, plan, _ := bundle.ValidateTransactionBundle(tx, pool.signer)
...
func (pool *TxPool) requestReset(oldHead *coretypes.EvmHeader, newHead *coretypes.EvmHeader) chan struct{} {
	select {
...
func (pool *TxPool) runReorg(done chan struct{}, reset *txpoolResetRequest, dirtyAccounts *accountSet, events map[common.Address]*txSortedMap) {
	...
	if len(events) > 0 {
		...
		pool.txFeed.Send(coretypes.NewTxsNotify{txs})
	}
}

// reset retrieves the current state of the blockchain and ensures the content
// of the transaction pool is valid with regard to the chain state.
func (pool *TxPool) reset(oldHead, newHead *coretypes.EvmHeader) {
...
func (pool *TxPool) createSubsidiesChecker() coresubsidies.SubsidiesChecker {
...
func (pool *TxPool) createCachedSubsidiesChecker() coresubsidies.SubsidiesChecker {
	return pool.subsidiesCheckerCache.Wrap(pool.createSubsidiesChecker())
}

func (pool *TxPool) createBundleChecker() corebundles.BundleChecker {
...
func (pool *TxPool) promoteExecutables(accounts []common.Address) []*types.Transaction {
	...
	bundleChecker := pool.createBundleChecker()
	...
	for _, addr := range accounts {
		...
		drops, _ := list.Filter(utils.Uint256ToBigInt(pool.currentState.GetBalance(addr)), pool.currentMaxGas, subsidiesChecker, bundleChecker)
...
func (pool *TxPool) demoteUnexecutables() {
	...
	for addr, list := range pool.pending {
		...
		drops, invalids := list.Filter(utils.Uint256ToBigInt(pool.currentState.GetBalance(addr)), pool.currentMaxGas, subsidiesChecker, bundleChecker)
		for _, tx := range drops {
			hash := tx.Hash()
