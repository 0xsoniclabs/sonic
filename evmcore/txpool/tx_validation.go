...
//go:generate mockgen -source=tx_validation.go -destination=tx_validation_mock.go -package=txpool
...
func validateTx(
	...
	subsidiesChecker coresubsidies.SubsidiesChecker,
	signer types.Signer,
) error {
...
func ValidateTxForNetwork(tx *types.Transaction, rules NetworkRules, chain coretypes.StateReader, signer types.Signer) error {
...
func ValidateTxForBlock(tx *types.Transaction, netRules NetworkRules, chain coretypes.StateReader) error {
...
func validateBundleTransactions(
	...
	netRules NetworkRules,
	signer types.Signer,
	chainState coretypes.StateReader,
...
func validateBundleTransactionsInternal(
	...
	chainAdapter := corebundles.NewPreCheckChainAdapter(chainState, stateDb)
	state := getBundleState(chainAdapter, tx)
	if state == corebundles.BundleStatePermanentlyBlocked {
