...
func TestTxPool_SponsoredTransactionsAreIncludedInThePendingSet(t *testing.T) {
	...
	subsidiesCheckerMock := coresubsidies.NewMockSubsidiesChecker(ctrl)
	subsidiesCheckerMock.EXPECT().IsSponsored(gomock.Any()).Return(true).AnyTimes()

	// Instantiate the pool
	factory := func(opera.Rules, coretypes.StateReader, state.StateDB, types.Signer) coresubsidies.SubsidiesChecker {
		return subsidiesCheckerMock
	}
	pool := newTxPool(poolConfig, chainConfig, chain, factory, corebundles.NewBundleChecker)
...
func mockChain(ctrl *gomock.Controller, chainConfig *params.ChainConfig, upgrades opera.Upgrades) *coretypes.MockStateReader {
	...
	chain.EXPECT().CurrentBlock().Return(&coretypes.EvmBlock{
...
//go:generate mockgen -source=tx_pool_subsidies_test.go -destination=tx_pool_subsidies_test_mock.go -package=txpool
