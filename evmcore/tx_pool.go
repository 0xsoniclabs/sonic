...
type PriorityLookup interface {
	// IsPrioritized reports whether the transaction with the given hash is
	// prioritized. A transaction that has not been classified yet is not
	// prioritized.
	IsPrioritized(hash common.Hash) bool
}
...
func (pool *TxPool) isPrioritized(tx *types.Transaction) bool {
	...
	return pool.priorityLookup.IsPrioritized(tx.Hash())
}
