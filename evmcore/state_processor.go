...
func (p *StateProcessor) Process(
	...
	return runTransactions(executionContext{
		signer:      signer,
		baseFee:     vmenv.Context.BaseFee,
		statedb:     statedb,
		gasPool:     gp,
		blockNumber: blockNumber,
		usedGas:     usedGas,
		runner:      transactionRunner{vmenv},
		onNewLog:    onNewLog,
		upgrades:    p.upgrades,
...
func runTransactions(
	...
	processed := make([]ProcessedTransaction, 0, 2*len(transactions))
	for _, tx := range transactions {
		processed = append(processed, runTransaction(
			context, tx, len(processed)+txIndexOffset,
		)...)
	}
	return processed
}
...
func runTransaction(
	...
	if context.upgrades.GasSubsidies && subsidies.IsSponsorshipRequest(tx) {
		return context.runner.runSponsoredTransaction(context, tx, txIndex)
	}
	return []ProcessedTransaction{
		context.runner.runRegularTransaction(context, tx, txIndex),
	}
}
...
type _transactionRunner interface {
	...
	runRegularTransaction(executionContext, *types.Transaction, int) ProcessedTransaction
	runSponsoredTransaction(executionContext, *types.Transaction, int) []ProcessedTransaction

	runWithBaseFeeCheck(executionContext, *types.Transaction, int) ProcessedTransaction
	runWithoutBaseFeeCheck(executionContext, *types.Transaction, int) ProcessedTransaction
}
...
func (r transactionRunner) Call(
	...
	return r.vmenv.Call(from, to, input, gas, value)
}
...
func (r transactionRunner) runRegularTransaction(
	...
	return r.runWithBaseFeeCheck(context, tx, txIndex)
}
...
func (r transactionRunner) runSponsoredTransaction(
	...
	return runSponsoredTransaction(context, tx, txIndex)
}

func (r transactionRunner) runWithBaseFeeCheck(
	context executionContext,
	tx *types.Transaction,
	txIndex int,
) ProcessedTransaction {
	return r.runTransaction(context, tx, txIndex, true)
}

func (r transactionRunner) runWithoutBaseFeeCheck(
	context executionContext,
	tx *types.Transaction,
	txIndex int,
) ProcessedTransaction {
	return r.runTransaction(context, tx, txIndex, false)
}
...
func (r transactionRunner) runTransaction(
	...
	msg, err := TxAsMessage(tx, context.signer, r.vmenv.Context.BaseFee)
	if err != nil {
		log.Info("Failed to convert transaction to message", "tx", tx.Hash().Hex(), "err", err)
		return ProcessedTransaction{Transaction: tx}
	}

	r.vmenv.Config.NoBaseFee = !checkBaseFee
	context.statedb.SetTxContext(tx.Hash(), txIndex)
	receipt, _, err := applyTransaction(
		msg, context.gasPool, context.statedb, context.blockNumber, tx,
		context.usedGas, r.vmenv, context.onNewLog,
	)
	if err != nil {
...
func runSponsoredTransaction(
	...
	snapshot := context.statedb.Snapshot()
	covered, fundId, err := subsidies.IsCovered(
		context.upgrades, context.runner, context.signer, tx, context.baseFee,
	)
	context.statedb.RevertToSnapshot(snapshot)
	if err != nil {
	...
	processed := context.runner.runWithoutBaseFeeCheck(context, tx, txIndex)
	...
	feeChargingTx, err := subsidies.GetFeeChargeTransaction(
		context.statedb, fundId, gasUsed, context.baseFee,
	)
	if err != nil {
	...
	return []ProcessedTransaction{
		processed,
		context.runner.runWithoutBaseFeeCheck(context, feeChargingTx, txIndex+1),
	}
}
...
func (tp *TransactionProcessor) Run(i int, tx *types.Transaction) []ProcessedTransaction {
	return runTransactions(executionContext{
		signer:      tp.signer,
		baseFee:     tp.vmEnvironment.Context.BaseFee,
		statedb:     tp.stateDb,
		gasPool:     tp.gp,
		blockNumber: tp.blockNumber,
		usedGas:     &tp.usedGas,
		runner:      transactionRunner{tp.vmEnvironment},
		onNewLog:    tp.onNewLog,
		upgrades:    tp.upgrades,
