...
//go:generate mockgen -source=subsidies_integration.go -destination=subsidies_integration_mock.go -package=coresubsidies
...
func (s *SubsidiesIntegrationImplementation) IsSponsored(tx *types.Transaction) bool {
	...
	blockContext := coreevm.NewEVMBlockContext(currentBlock.Header(), s.chain, nil)
