...
// This is a modified version of the original stress tool from
// https://pkg.go.dev/golang.org/x/tools/cmd/stress#section-documentation
// The modifications are to run with race detector enabled by default
// and print the flags.
...
var (
	...
	flagRace    = flag.Bool("race", true, "enable race detector")
)
	...
	for i := 0; i < *flagP; i++ {
		go func() {
			for {
				...
				if started.Add(1) > int64(*flagCount) && *flagCount > 0 { // no:lint
					break
				}
