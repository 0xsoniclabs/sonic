...
func testSponsoring(t *testing.T, net IntegrationTestNetSession) {
	...
	sponsor := makeAccountWithBalance(t, net, big.NewInt(1e17))
...
func testBatching(t *testing.T, net IntegrationTestNetSession) {
	...
	sender := makeAccountWithBalance(t, net, big.NewInt(1e17)) // < pays transaction and transfers funds
	receiver1 := makeAccountWithBalance(t, net, big.NewInt(0))
	receiver2 := makeAccountWithBalance(t, net, big.NewInt(0))
	...
	assert.Equal(t,
		new(big.Int).Sub(
			big.NewInt(1e17), effectiveCost), balance)
...
func testPrivilegeDeescalation(t *testing.T, net IntegrationTestNetSession) {
	...
	account := makeAccountWithBalance(t, net, big.NewInt(1e17))     // < will transfer funds
	userAccount := makeAccountWithBalance(t, net, big.NewInt(1e17)) // < will pay for gas
	receiver := makeAccountWithBalance(t, net, big.NewInt(0))       // < will receive funds
	...
	unauthorizedAccount := makeAccountWithBalance(t, net, big.NewInt(1e17))
...
func testDelegateCanBeSetAndUnset(t *testing.T, net IntegrationTestNetSession) {

	client, err := net.GetClient()
	require.NoError(t, err)
	defer client.Close()

	account := makeAccountWithBalance(t, net, big.NewInt(1e17))
...
func testAuthorizationsAreExecutedInOrder(t *testing.T, net IntegrationTestNetSession) {
...
func testMultipleAccountsCanSubmitAuthorizations(t *testing.T, net IntegrationTestNetSession) {
	...
	account := makeAccountWithBalance(t, net, big.NewInt(1e17)) // < Pays for the transaction
	nonce, err := client.NonceAt(context.Background(), account.Address(), nil)
	require.NoError(t, err, "failed to get nonce for account", account.Address())
...
func testAuthorizationSucceedsWithFailingTx(t *testing.T, net IntegrationTestNetSession) {
...
func testAuthorizationFromNonExistingAccount(t *testing.T, net IntegrationTestNetSession) {

	client, err := net.GetClient()
	require.NoError(t, err)
	defer client.Close()

	sponsor := makeAccountWithBalance(t, net, big.NewInt(1e17)) // < Pays for the transaction
...
func testNoDelegateToDelegated(t *testing.T, net IntegrationTestNetSession) {
...
func testChainOfCalls(t *testing.T, net IntegrationTestNetSession) {
	...
	sponsor := makeAccountWithBalance(t, net, big.NewInt(1e17)) // < Pays for the transaction gas, originates the call-chain
	account1 := NewAccount()
	account2 := NewAccount()
...
func getCallData(t *testing.T, net IntegrationTestNetSession,
	transactionConstructor func(*bind.TransactOpts) (*types.Transaction, error)) []byte {
	txOpts, err := net.GetTransactOptions(net.GetAccount())
	require.NoError(t, err)
