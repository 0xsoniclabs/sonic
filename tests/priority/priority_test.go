...
func testPrioritiesScheduledFirst(t *testing.T, singleProposer bool) {
	...
	for i := range prios {
		...
		prios[i].account = acc
		setPrioritized(t, net, acc.Address(), prios[i].level, prios[i].weight, uint64(i+1))
	}
	...
	type prioEntry struct{ level, weight uint64 }
	...
	// available before the emitter builds its first event; submitting them via
	// separate parallel RPCs would let the emitter form an event mid-arrival,
	// splitting the prioritized transactions across blocks.
...
// TestPriority_PriorityOrderingPreservesNonceOrdering verifies the per-sender
// nonce guarantee of block-formation reordering (see §11 of the priorities
// design doc): a transaction keeps its priority only while it extends its
// sender's contiguous run of prioritized nonces from the block-start account
// nonce. A prioritized transaction whose lower-nonce predecessor is not
// prioritized is therefore demoted to its base-order position rather than
// hoisted ahead of that predecessor, where it would be nonce-too-high and
// skipped. Both transactions consequently execute in nonce order, in the same
// block, whether or not the sender is registered as prioritized.
func TestPriority_PriorityOrderingPreservesNonceOrdering(t *testing.T) {
	require := require.New(t)
	...
	cases := map[string]struct {
		...
		"prioritized sender": {prioritized: true},
		"ordinary sender":    {prioritized: false},
	}
	...
	for name, tc := range cases {
		t.Run(name, func(t *testing.T) {
			...
			require.Equal(receipt1.BlockNumber.Uint64(), receipt2.BlockNumber.Uint64(),
				"both txs must land in the same block")
			require.Less(receipt1.TransactionIndex, receipt2.TransactionIndex,
				"tx1 (lower nonce) must precede tx2")
		})
	}
}
