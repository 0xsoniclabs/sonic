...
func TestPriority_PriorityCanBeEnabledAndDisabled(t *testing.T) {
	...
	for name, upgrade := range opera.GetAllHardForksInOrder() {
		t.Run(name, func(t *testing.T) {
			...
			setPrioritized(t, net, prio.Address(), 1, 1, 0xaa)
...
// TODO we can not test that the tx is not prioritized if the registry is not deployed, because we can not set the sender to be prioritized and therefore we dont see any difference to a deployed registry.
...
// are appended last, and asserts on the resulting global ordering:
//   - expectPrioritized == true: all of `prio`'s transactions come before every
//     other user transaction.
//   - expectPrioritized == false: at least one non-`prio` user transaction
//     precedes a `prio` transaction (proof that no reordering took place).
//
...
func requirePriorityHasEffect(
	...
	// Add the whole burst to the pool in a single batch so ordinary and prio
	// txs all become available before the emitter builds its first event,
	// making their relative ordering deterministic.
	hashes, err := net.SendAllToPool(t.Context(), burst)
	require.NoError(err)
	waitForReceipts(t, net, hashes)

	prioAddr := prio.Address()
	requirePriorityAppliedSince(t, net, signer, afterBlock, expectPrioritized,
