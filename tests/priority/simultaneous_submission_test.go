...
func runDuel(
	...
	// The endowment of ordAcc is confirmed on the node it was submitted to,
	// which is not necessarily ordNode. Wait for it to become visible there,
	// otherwise ordNode's transaction pool still sees an empty account and
	// rejects ordTx for insufficient funds.
	require.NoError(tests.WaitFor(t.Context(), func(ctx context.Context) (bool, error) {
		balance, err := ordClient.BalanceAt(ctx, ordAcc.Address(), nil)
		if err != nil {
			return false, err
		}
