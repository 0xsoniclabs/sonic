...
func setPrioritized(
	...
	receipt, err := net.Apply(func(opts *bind.TransactOpts) (*types.Transaction, error) {
		return reg.SetSenderPriority(opts, sender, big.NewInt(level), big.NewInt(weight), id)
	})
	require.NoError(err)
	require.Equal(types.ReceiptStatusSuccessful, receipt.Status)
}
...
// requirePriorityAppliedSince inspects every block from `firstBlock` onward and
// asserts on the ordering of prioritized (per `isPrioritized`) vs ordinary
// user transactions. In every case it requires at least one block that mixes
// both classes so the check is meaningful.
//
//   - expectPrioritized == true: within every mixed block, prioritized txs
//     must form a prefix (appear before any ordinary tx).
//   - expectPrioritized == false: at least one mixed block must place an
//     ordinary tx before a prioritized one (proof that no reordering ran).
func requirePriorityAppliedSince(
	...
	mixSeen := false
	ordinaryBeforePrioritySeen := false
	for n := firstBlock; n <= latest; n++ {
		...
		for _, tx := range block.Transactions() {
			...
			if isPrioritized(sender) {
				if sawOrdinary {
					ordinaryBeforePrioritySeen = true
				}
				sawPriority = true
			} else {
				sawOrdinary = true
		...
		if sawPriority && sawOrdinary {
			mixSeen = true
		}
	}
	require.True(mixSeen,
		"expected at least one block mixing prioritized and ordinary transactions")
	if expectPrioritized {
		require.False(ordinaryBeforePrioritySeen,
			"with priorities enabled, no block must place an ordinary tx before a prioritized tx")
	} else {
		require.True(ordinaryBeforePrioritySeen,
			"with priorities disabled, expected at least one block to place an ordinary tx before a prioritized tx")
	}
}
