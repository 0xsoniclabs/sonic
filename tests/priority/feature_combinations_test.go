...
func TestPriority_PriorityAndBundles(t *testing.T) {
	...
	setPrioritized(t, net, envelopeSender.Address(), 1, 1, 1)
	...
	// Add the bundle envelope and every ordinary tx to the pool in a single
	// batch so they all become available before the emitter builds its first
	// event, making their relative ordering deterministic.
	...
	requirePriorityAppliedSince(t, net, signer, afterBlock, true,
...
func TestPriority_PriorityAndSponsorship(t *testing.T) {
	...
	setPrioritized(t, net, sponsee.Address(), 1, 1, 2)
	...
	// Add the sponsored tx and every ordinary tx to the pool in a single batch
	// so they all become available before the emitter builds its first event,
