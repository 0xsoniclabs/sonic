...
// ReplayResult holds the results of replaying a chain, specifically the
// receipts generated for each block's transactions.
type ReplayResult struct {
	// Receipts maps block number to a map of tx hash to receipt.
	Receipts map[uint64]map[common.Hash]*types.Receipt
}
...
func ReplayChain(
	...
	for i, block := range rawBlocks {
		...
		// Determine difficulty for block context
		...
		if len(block.Transactions()) > 0 {
			receipts, err := replayBlockTransactions(
				statedb, chainConfig, evmHeader, chain, block, difficulty,
			)
			if err != nil {
				return nil, fmt.Errorf("replaying block %d (index %d): %w", blockNum, i, err)
			}
			result.Receipts[blockNum] = receipts
		}
...
func replayBlockTransactions(
	...
	blockContext := evmcore.NewEVMBlockContextWithDifficulty(
		evmHeader, chain, nil, new(big.Int).Set(difficulty),
	)
