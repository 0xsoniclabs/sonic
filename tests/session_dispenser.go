...
// getSession retrieves or creates a new session for network running on the
// given Upgrade. If a tests can run in parallel, the call to t.Parallel()
...
func hashOptions(upgrades opera.Upgrades) common.Hash {
	...
	fmt.Printf("Serialized options: %s\n", jsonData)
	h.Write(jsonData)
