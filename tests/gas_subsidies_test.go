...
func testCanRunSubsidizedTransactions(t *testing.T, singleProposer bool) {
	...
	_, globalFund, err := registry.GlobalSponsorshipFundId(nil)
	require.NoError(err)
	receipt, err = net.Apply(func(opts *bind.TransactOpts) (*types.Transaction, error) {
		opts.Value = big.NewInt(1e16)
		return registry.Sponsor(opts, globalFund)
	})
	require.NoError(err)
	require.Equal(types.ReceiptStatusSuccessful, receipt.Status)
	...
	sponsorship, err := registry.Sponsorships(nil, globalFund)
	require.NoError(err)
	...
	sponsorship, err = registry.Sponsorships(ops, globalFund)
	require.NoError(err)
