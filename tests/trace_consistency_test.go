...
func TestTraceConsistency_TracesReproduceBlockProcessing(t *testing.T) {
	...
	// eth_simulateV1, in contrast, builds a block that never existed on the
	// real chain, so its history storage slot exists only if eth_simulateV1's
	// block builder applies the pre-block system call itself. Simulating on
	// top of the parent block reproduces block receipt.BlockNumber as the
	// first (and only) simulated block.
	t.Run("eth_simulateV1", func(t *testing.T) {
		result := simulateV1HistoryStorage(t, rpcClient, queriedBlockNumber, queriedBlockNumber)
		req.New(t).Equal(observedDuringBlockProcessing, result)
	})
...
// historyStorageCallData encodes the argument the EIP-2935 history storage
// contract expects: the queried block number, left-padded to 32 bytes, with
// no function selector.
func historyStorageCallData(queriedBlock *big.Int) hexutil.Bytes {
	return common.LeftPadBytes(queriedBlock.Bytes(), 32)
}

// ethCallHistoryStorage issues an eth_call directly against the history
// storage contract, as of the given block, and returns the block hash it reports.
func ethCallHistoryStorage(t *testing.T, rpcClient *rpc.Client, blockNumber, queriedBlock *big.Int) common.Hash {
	t.Helper()
	data := historyStorageCallData(queriedBlock)
	args := ethapi.TransactionArgs{
		To:   &params.HistoryStorageAddress,
		Data: &data,
	}
	result := callRPC[hexutil.Bytes](t, rpcClient, "eth_call", args,
		rpc.BlockNumberOrHashWithNumber(rpc.BlockNumber(blockNumber.Int64())))
	return common.BytesToHash(result)
}

// simulateV1CallResult is the subset of an eth_simulateV1 call result needed
...
// simulateV1HistoryStorage issues an eth_simulateV1 call that simulates a
// single block on top of the given base block, calling the history storage
// contract, and returns the block hash it reports.
func simulateV1HistoryStorage(t *testing.T, rpcClient *rpc.Client, baseBlockNumber, queriedBlock *big.Int) common.Hash {
	t.Helper()
	require := req.New(t)
	...
	results := callRPC[[]simulateV1BlockResult](t, rpcClient, "eth_simulateV1", opts,
		rpc.BlockNumberOrHashWithNumber(rpc.BlockNumber(baseBlockNumber.Int64())))
	require.Len(results, 1)
	require.Len(results[0].Calls, 1)
	return common.BytesToHash(results[0].Calls[0].ReturnValue)
}
