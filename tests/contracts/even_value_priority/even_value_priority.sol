...
contract EvenValuePriority {
    ...
    uint256 constant PRIORITY_LEVEL = 1;

    // A fixed weight assigned to prioritized txs.
    uint256 constant PRIORITY_WEIGHT = 0;
    ...
    function getPriority(
        ...
        bytes calldata /*data*/,
        uint256 /*gas*/
    ) external pure returns (uint256 level, uint256 weight, bytes32 id) {
        if (value % 2 == 0) {
            return (PRIORITY_LEVEL, PRIORITY_WEIGHT, bytes32(0));
        }
