...
// TestNetworkSponsorship_Mode2_NoSecondTransaction verifies that a mode-2 sponsored
// transaction produces exactly one receipt in the block and no deduction/track tx.
func TestNetworkSponsorship_Mode2_NoSecondTransaction(t *testing.T) {
	net := startNetWithSubsidies(t)

	// Install the mode-2 registry (network sponsored, no tracking).
	...
	_, err = net.EndowAccount(sender.Address(), big.NewInt(1)) // send 1 wei so sender exists
	require.NoError(t, err)
	...
	for i, blockTx := range block.Transactions() {
		...
		// Mode 2: must be the only tx or, if there are more, the next one
		...
		if i+1 < len(block.Transactions()) {
			...
			require.False(t,
				...
				"mode 2 should not insert a registry call after the sponsored tx",
			)
		}

		// Gas used in block must not include overhead from a second tx.
		// The sponsored tx's gas used should equal its receipt's gas used.
		require.Equal(t, receipt.GasUsed, receipt.CumulativeGasUsed,
			"mode 2 tx gas used should equal cumulative gas (no preceding sponsored txs in this block)")
		break
	}
	require.True(t, found, "sponsored tx not found in block")
}

// TestNetworkSponsorship_Mode3_TrackTransactionInserted verifies that a mode-3
// sponsored transaction produces two receipts: the sponsored tx and a track tx,
// and that the track tx carries the correct tracking ID and fee.
func TestNetworkSponsorship_Mode3_TrackTransactionInserted(t *testing.T) {
	net := startNetWithSubsidies(t)

	// Install the mode-3 registry (network sponsored with tracking).
	...
	sender := tests.NewAccount()
	_, err = net.EndowAccount(sender.Address(), big.NewInt(1))
	require.NoError(t, err)
...
// TestNetworkSponsorship_LegacyChooseFund_32ByteResponse_Mode1Behavior verifies
// that a registry returning a legacy 32-byte chooseFund response is treated as
// mode 1 (fund-backed), with a deductFees tx appended after execution.
func TestNetworkSponsorship_LegacyChooseFund_32ByteResponse_Mode1Behavior(t *testing.T) {
	net := startNetWithSubsidies(t)
	...
	// Check that a deductFees tx follows the sponsored tx (mode 1 behavior).
	found := false
	for i, blockTx := range block.Transactions() {
		if blockTx.Hash() != tx.Hash() {
			continue
		...
		require.True(t, subsidies.IsFeeChargeTransaction(deductTx),
			"second tx must be a fee charge (deductFees) transaction for mode 1")
		break
	}
	require.True(t, found, "sponsored tx not found in block")
}
