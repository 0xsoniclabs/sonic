...
```json
{
  ...
  "steps": [
    // Phase 1: user sponsors the temporary account
    {
      "from": "0xUser",
      "to": "0xTemp",
      "nonce": "0x10",
...
**Gasless onboarding.** A new user with only a token balance and no native S for gas can authorize a bundle in which a relayer covers gas. The relayer is reimbursed from the user's token balance within the same bundle. Note that in this case the order is reversed: the relayer's transactions come first (covering gas), and the user's signed reimbursement transaction follows -- the same atomicity guarantee still ensures the relayer cannot collect without completing its side of the deal.
