...
func NewMessage(msg *core.Message, txType uint8) *substate.Message {
	...
	setCodeAuthorizations := make([]stypes.SetCodeAuthorization, len(msg.SetCodeAuthorizations))
	for i, authorization := range msg.SetCodeAuthorizations {
		setCodeAuthorizations[i] = stypes.SetCodeAuthorization{
			ChainID: authorization.ChainID,
			Address: stypes.Address(authorization.Address),
