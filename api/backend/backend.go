...
// Package backend defines the Backend interface through which the RPC API
// handlers access the node. It is a separate package so that API handler
// packages and testing utilities (like rpctest) can share it without
// import cycles.
package backend
...
//
//go:generate mockgen -source=backend.go -destination=../ethapi/backend_mock.go -package=ethapi
