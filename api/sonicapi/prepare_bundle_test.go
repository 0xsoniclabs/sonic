...
func Test_PrepareBundle_CreatesValidTransactions(t *testing.T) {
	...
	x, err := api.PrepareBundle(t.Context(),
		PrepareBundleArgs{

			RPCExecutionProposal: RPCExecutionProposal{
				BlockRange: RPCRange{
					Earliest: hexutil.Uint64(12345678),
					Latest:   hexutil.Uint64(12345778),
				},
					...
					Steps: []any{
						RPCExecutionStepProposal{
							TransactionArgs: ethapi.TransactionArgs{
								...
								Nonce: toPtr(hexutil.Uint64(1)),
								Gas:   toPtr(hexutil.Uint64(21000)),
...
func Test_PrepareBundle_LooksFlat(t *testing.T) {
	...
	var v RPCExecutionProposal
	json.Unmarshal([]byte(flatProposal), v)
	...
	result, err := api.PrepareBundle(t.Context(), PrepareBundleArgs{
		RPCExecutionProposal: v,
	})
	require.NoError(t, err)
...
func Test_convertProposalToPlanInternal(t *testing.T) {
	...
	txRequest := ethapi.TransactionArgs{
		From:  &sender,
		To:    &common.Address{2},
		Nonce: toPtr(hexutil.Uint64(1)),
		Gas:   toPtr(hexutil.Uint64(21000)),
	...
	got, err := convertProposalToPlan(signer, proposal)
	require.NoError(t, err)
