...
type PeerProgress = backend.PeerProgress

// Backend interface provides the common API services (that are provided by
// both full and light clients) with access to necessary functions.
//
// The interface is defined in the api/backend package; the alias keeps the
// name available to the ethapi package and its consumers.
type Backend = backend.Backend

// GetBlockContext returns a new vm.BlockContext based on the given header and backend
func GetBlockContext(ctx context.Context, b Backend, header *evmcore.EvmHeader) vm.BlockContext {
	return backend.GetBlockContext(ctx, b, header)
}
...
func GetVmConfig(
	...
	rules, err := b.GetNetworkRules(ctx, blockHeight)
	if err != nil {
