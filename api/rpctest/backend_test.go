...
func Test_RulesFromChainConfig_RejectsNilConfig(t *testing.T) {
	_, err := UpgradesFromChainConfigAndHistory(nil, nil)
	require.ErrorContains(t, err, "chain config is nil")
}

func Test_RulesFromChainConfig_RejectsNilChainID(t *testing.T) {
	_, err := UpgradesFromChainConfigAndHistory(&params.ChainConfig{}, nil)
...
func Test_RulesFromChainConfig_MapsHardForksToUpgrades(t *testing.T) {
	...
	rules, err := UpgradesFromChainConfigAndHistory(cfg, nil)
	require.NoError(t, err)

	up, ok := rules[0]
	require.True(t, ok)
	require.True(t, up.Berlin)
	require.True(t, up.London)
	require.True(t, up.Sonic)
	require.False(t, up.Allegro)
	require.False(t, up.Brio)
	require.False(t, up.Llr)
}
...
func Test_NewBackendBuilder_BuildFromReplay_UsesChainConfigUpgrades(t *testing.T) {
	...
	require.False(t, rules.Upgrades.Allegro)
