...
> The demo keys are derived from the fake keys following the validator keys, so
> changing `VALIDATORS` also changes the demo accounts listed in [§5](#5-what-you-get).
> The startup banner always prints the accounts actually in use.
...
// Optional: per-entity limits — total gas of prioritized transactions per entity
// per block, and how many prioritized transactions per entity a validator
// eagerly packs into one event. Zero keeps the defaults (10,000,000 and 4).
...
- **Reset:** the network is recreated from a fresh genesis on every start, so
  just start it again. (For an unchanged `VALIDATORS`, the demo account
  addresses and keys are always the same.)
...
- **A demo transaction is not prioritized** — make sure the sender was
  registered with a non-zero `level`. A registered sender is also skipped if its
  nonce does not continue the sender's sequence from the block-start nonce (a
  gap leaves that transaction and all later ones behind), or if the entity's
  prioritized transactions would exceed the per-entity gas budget for the
  block (10,000,000 gas unless changed via `setConfig`).
