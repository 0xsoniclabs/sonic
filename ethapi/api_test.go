...
func TestMakeConfigFromUpgrade_Reports_AvailableSystemContracts(t *testing.T) {
	...
	tests := map[string]struct {
		...
		"Sonic": {
			blockNumber:      big.NewInt(int64(sonicHeight) + 1),
			wantSysContracts: contractRegistry{},
			wantForkId:       hexutil.Bytes{0x66, 0x42, 0x1a, 0x82},
		},
		"Allegro": {
			blockNumber:      big.NewInt(int64(allegroHeight) + 1),
		...
		"GasSubsidies": {
			blockNumber:      big.NewInt(int64(gasSubsidiesHeight) + 1),
	...
	for name, test := range tests {
		t.Run(name, func(t *testing.T) {
			...
			result, err := makeConfigFromUpgrade(context.Background(), backend, upgradeHeights, test.blockNumber)
